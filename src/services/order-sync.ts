...
import { SpApiOrder, SpApiOrderItem, SyncResult } from '../types';
import logger from './logger';

const prisma = new PrismaClient();

export class OrderSyncService {
  constructor(private spApiClient: SpApiClient) {}

  async syncUnshippedOrders(): Promise<SyncResult> {
    ...
    try {
      ...
      do {
        ...
        if (response.Orders) allOrders.push(...response.Orders);
        nextToken = response.NextToken;
      } while (nextToken);

      logger.info(`Found ${allOrders.length} unshipped orders`);
  ...
  private async getLastSyncTime(): Promise<string> {
    ...
    if (lastSync?.startedAt) return lastSync.startedAt.toISOString();
  ...
  private async upsertOrder(order: SpApiOrder): Promise<void> {
  ...
  private async syncOrderItems(amazonOrderId: string): Promise<number> {
    ...
    do {
      ...
      if (response.OrderItems) allItems.push(...response.OrderItems);
      nextToken = response.NextToken;
    } while (nextToken);
